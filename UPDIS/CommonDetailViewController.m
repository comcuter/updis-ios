...
-(void)initHeadView
{
    ...
    //查看全部
    self.btnShowAll = [self createButtonWithTitle:@"查看全部" image:nil imageH:nil];
...
//    [self.toolBarView addSubview:self.btnWriteComment];
    ...
    if (!self.inputView) {
        ...
        [self.inputView.selectButton addTarget:self action:@selector(selectedPressed:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)contentLoadOver:(MessageDataModel *)dataModel
{
    ...
    NSString *path = [[[NSBundle mainBundle] bundlePath] stringByAppendingPathComponent:@"content.htm"];
    NSString *templateStr = [[NSString alloc] initWithContentsOfFile:path encoding:NSUTF8StringEncoding error:nil];
    templateStr = [templateStr stringByReplacingOccurrencesOfString:@"$title" withString:dataModel.title];
    templateStr = [templateStr stringByReplacingOccurrencesOfString:@"$custlist" withString:dataModel.messageDetailMeta];
    templateStr = [templateStr stringByReplacingOccurrencesOfString:@"$content" withString:dataModel.content];
...
- (void)requestDidFinishLoad:(TTURLRequest*)request
{
    ...
    if ([[feed objectForKey:@"success"] integerValue]) {
        ...
        [self.commonDetailDataSource.commonDetailDataSourceModel load:TTURLRequestCachePolicyNone more:NO];
    }
}
